...
<body>
    ...
    <script>
    // Tracking de durée d'utilisation du module Team Battle
    (function(){
        const token = '<?= htmlspecialchars($token, ENT_QUOTES) ?>';
        let sessionStartTs = Date.now();
        fetch(`/modules/profile_manager.php?action=battle_session_start&token=${encodeURIComponent(token)}`, {method: 'POST'}).catch(()=>{});
        function sendDuration(){
            const seconds = Math.max(0, Math.round((Date.now() - sessionStartTs)/1000));
            navigator.sendBeacon ?
                navigator.sendBeacon(`/modules/profile_manager.php?action=battle_session_end&token=${encodeURIComponent(token)}`, new Blob([JSON.stringify({duration: seconds})], {type:'application/json'}))
                : fetch(`/modules/profile_manager.php?action=battle_session_end&token=${encodeURIComponent(token)}`, {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({duration: seconds})}).catch(()=>{});
        }
